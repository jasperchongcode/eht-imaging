...
def make_amp(obs, debias=True, polarization='unknown', band='unknown', round_s=0.1):
    ...
    if debias == True:
        amp2 = np.maximum(np.asarray(
            df['amp'])**2-np.asarray(df['sigma'])**2, np.asarray(df['sigma'])**2)
        df['amp'] = np.sqrt(amp2)
...
def coh_avg_vis(obs, dt=0, scan_avg=False, return_type='rec', err_type='predicted', num_samples=int(1e3)):
    ...
    if (dt <= 0) & (scan_avg == False):
        ...
        if scan_avg == False:
            ...
            grouping = ['tau1', 'tau2', 'polarization',
                        'band', 'baseline', 't1', 't2', 'round_time']
        else:
            bins, labs = get_bins_labels(obs.scans)
            vis['scan'] = list(pd.cut(vis.time, bins, labels=labs))
            grouping = ['tau1', 'tau2', 'polarization',
                        'band', 'baseline', 't1', 't2', 'scan']
        ...
        if err_type == 'measured':
            ...
            def meanerrF(x): return bootstrap(np.abs(x), np.mean,
                                              num_samples=num_samples, wrapping_variable=False)
        ...
        if scan_avg == False:
            ...
            half_bucket = dt/2.
            vis_avg['datetime'] = list(map(
                lambda x: t0 + datetime.timedelta(seconds=int(dt*x) + half_bucket), vis_avg['round_time']))
            vis_avg['time'] = list(
        ...
        if err_type == 'measured':
            vis_avg.drop(labels=['udummy', 'vdummy', 'qdummy',
                         'dummy'], axis='columns', inplace=True)
        if return_type == 'rec':
            if obs.polrep == 'stokes':
...
def coh_moving_avg_vis(obs, dt=50, return_type='rec'):
    ...
    vis['roll_vis'] = list(zip(vis['total_seconds'], vis[vis1],
                           vis[vis2], vis[vis3], vis[vis4], vis['datetime']))
    vis['roll_sig'] = list(zip(vis['total_seconds'], vis[sig1],
                           vis[sig2], vis[sig3], vis[sig4], vis['datetime']))

    def roll_vis_local(x): return roll_vis(
        x, dt=str(int(dt))+'s', min_periods=min_periods)
    def roll_sig_local(x): return roll_sig(
        x, dt=str(int(dt))+'s', min_periods=min_periods)
    vis_avg_roll_vis = vis[['baseline', 'roll_vis']].groupby(
        'baseline').transform(roll_vis_local)['roll_vis'].copy()
    vis_avg_roll_sig = vis[['baseline', 'roll_sig']].groupby(
        'baseline').transform(roll_sig_local)['roll_sig'].copy()
    ...
    vis.datetime = vis.datetime.apply(
        lambda x: x-datetime.timedelta(seconds=dt))
...
def roll_vis(ser, dt='1s', min_periods=1):
    ...
    avg_list = list(zip(avg['REvis1'], avg['IMvis1'], avg['REvis2'], avg['IMvis2'],
                    avg['REvis3'], avg['IMvis3'], avg['REvis4'], avg['IMvis4'], [x[5] for x in ser]))
    return avg_list
...
def roll_sig(ser, dt='1s', min_periods=1):
    ...
    avg_list = list(zip(avg['sig1'], avg['sig2'],
                    avg['sig3'], avg['sig4'], [x[5] for x in ser]))
    return avg_list
...
def make_cphase_df(obs, band='unknown', polarization='unknown', mode='all', count='max', round_s=0.1, snrcut=0., uv_min=False):
    ...
    df['mjd'] = obs.mjd + df['fmjd']
    df['triangle'] = list(
        map(lambda x: x[0]+'-'+x[1]+'-'+x[2], zip(df['t1'], df['t2'], df['t3'])))
    df['datetime'] = Time(df['mjd'], format='mjd').datetime
    df['datetime'] = list(
        map(lambda x: round_time(x, round_s=round_s), df['datetime']))
    df['jd'] = Time(df['mjd'], format='mjd').jd
...
def make_cphase_diag_df(obs, vtype='vis', band='unknown', polarization='unknown', count='min', round_s=0.1, snrcut=0., uv_min=False):
    ...
    data = obs.c_phases_diag(vtype=vtype, count=count,
...
def make_camp_df(obs, ctype='logcamp', debias=False, band='unknown', polarization='unknown', mode='all', count='max', round_s=0.1, snrcut=0.):
    ...
    data = obs.c_amplitudes(mode=mode, count=count,
                            debias=debias, ctype=ctype, snrcut=snrcut)
    sour = obs.source
    df = pd.DataFrame(data=data).copy()
    df['fmjd'] = df['time']/24.
    df['mjd'] = obs.mjd + df['fmjd']
    df['quadrangle'] = list(map(lambda x: x[0]+'-'+x[1]+'-' +
                            x[2]+'-'+x[3], zip(df['t1'], df['t2'], df['t3'], df['t4'])))
    df['datetime'] = Time(df['mjd'], format='mjd').datetime
    df['datetime'] = list(
        map(lambda x: round_time(x, round_s=round_s), df['datetime']))
    df['jd'] = Time(df['mjd'], format='mjd').jd
...
def make_logcamp_diag_df(obs, debias=True, band='unknown', polarization='unknown', mode='all', count='min', round_s=0.1, snrcut=0.):
    ...
    data = obs.c_log_amplitudes_diag(
        mode=mode, count=count, debias=debias, snrcut=snrcut)
    sour = obs.source
...
def average_bispectra(cdf, dt, return_type='rec', num_samples=int(1e3), snrcut=0.):
    ...
    # snrcut
    cdf2 = cdf2[np.abs(cdf2['bispec']/cdf2['sigmab'])
...
def df_to_rec(df, product_type):
    ...
    if product_type == 'cphase':
        ...
        out = df[['time', 'tint', 't1', 't2', 'tau1', 'tau2', 'u', 'v', 'vis', 'qvis',
                  'uvis', 'vvis', 'sigma', 'qsigma', 'usigma', 'vsigma']].to_records(index=False)
        return np.array(out, dtype=DTPOL_STOKES)
    elif product_type == 'vis_circ':
        out = df[['time', 'tint', 't1', 't2', 'tau1', 'tau2', 'u', 'v', 'rrvis', 'llvis',
                  'rlvis', 'lrvis', 'rrsigma', 'llsigma', 'rlsigma', 'lrsigma']].to_records(index=False)
        ...
        out = df[['time', 'cphase', 'sigmacp', 'triangles',
                  'u', 'v', 'tform_matrix']].to_records(index=False)
        return np.array(out, dtype=DTCPHASEDIAG)
    elif product_type == 'logcamp_diag':
        out = df[['time', 'camp', 'sigmaca', 'quadrangles',
                  'u', 'v', 'tform_matrix']].to_records(index=False)
...
def get_bins_labels(intervals, dt=0.00001):
    ...
    def replace_overlapping_intervals(intervals, element_ind):
        indic_not_overlap = [not is_overlapping(
            x, intervals[element_ind]) for x in intervals]
        indic_overlap = [is_overlapping(
            x, intervals[element_ind]) for x in intervals]
    ...
    for frame in frames:
        ...
        if uniquely:
            frame.drop_duplicates(
                subset=['all_ind'], keep='first', inplace=True)
...
def add_gmst(df):
    ...
    if type(timestamps[0]) is np.datetime64:  # pandas < 0.9
        ...
        raise Exception(
            "do not know how to convert timestamp of type " + repr(type(timestamps[0])))
